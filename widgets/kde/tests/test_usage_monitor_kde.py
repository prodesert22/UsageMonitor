...
class UpdateTests(unittest.TestCase):
    ...
    def test_compare_versions_orders_releases(self):
        self.assertLess(um.compare_versions("0.8.0", "0.8.1"), 0)
        self.assertEqual(um.compare_versions("0.8.1", "0.8.1"), 0)
        self.assertEqual(um.compare_versions("0.8", "0.8.0"), 0)
        self.assertGreater(um.compare_versions("v0.9", "0.8.1"), 0)

    def test_is_outdated_ignores_missing_and_unknown(self):
        self.assertFalse(um.is_outdated("", "0.8.1"))
        self.assertFalse(um.is_outdated(None, "0.8.1"))
        self.assertFalse(um.is_outdated("0.8.0", "unknown"))
        self.assertFalse(um.is_outdated("0.8.1", "0.8.1"))
        self.assertFalse(um.is_outdated("0.9.0", "0.8.1"))
        self.assertTrue(um.is_outdated("0.8.0", "0.8.1"))

    def test_update_info_reports_outdated_with_release_url(self):
        info = um.update_info("0.8.0", "0.8.1", {})
    ...
    def test_update_info_honours_dismissal(self):
        info = um.update_info("0.8.0", "0.8.1", {"dismissedUpdateVersion": "0.8.1"})
        self.assertTrue(info["outdated"])
        self.assertTrue(info["dismissed"])
        current = um.update_info("0.8.1", "0.8.1", {})
    ...
    def _write_release_cache(self, version, age_seconds, payload):
        ...
        um.write_json(um._release_cache_path(version), {"fetchedAt": iso, "payload": payload})
    ...
    def test_fetch_changelog_falls_back_to_stale_cache_then_unavailable(self):
        ...
        with tempfile.TemporaryDirectory() as td, mock.patch.dict(os.environ, self._env(td), clear=True):
            ...
            self.assertEqual(um.fetch_changelog("0.8.1", runner=dead), stale)

    def test_apply_update_runs_widget_install_for_target(self):
        run = mock.Mock(return_value=proc("ok"))
        um.apply_update("kde", runner=run)
        self.assertEqual(run.call_args.args[0], ["widget", "install", "kde"])

    def test_update_dismiss_records_version_in_state(self):
        with tempfile.TemporaryDirectory() as td, mock.patch.dict(os.environ, self._env(td), clear=True):
            out = io.StringIO()
            with contextlib.redirect_stdout(out):
                um.command_update_dismiss(SimpleNamespace(version="0.8.1"))
            self.assertEqual(json.loads(out.getvalue())["dismissed"], "0.8.1")
            self.assertEqual(um.state_value(key="dismissedUpdateVersion"), "0.8.1")

    def test_fetch_changelog_survives_runner_crash(self):
        with tempfile.TemporaryDirectory() as td, mock.patch.dict(os.environ, self._env(td), clear=True):
            missing = um.fetch_changelog("0.8.1", runner=mock.Mock(side_effect=FileNotFoundError("no cli")))
            self.assertEqual(missing["source"], "unavailable")
            self.assertIn("/releases/tag/v0.8.1", missing["url"])

    def test_release_cache_path_cannot_escape_cache_dir(self):
        with tempfile.TemporaryDirectory() as td, mock.patch.dict(os.environ, self._env(td), clear=True):
            cache_dir = um.paths().cache_dir
            path = um._release_cache_path("../../evil")
    ...
    def test_fetch_changelog_caches_body_verbatim(self):
        ...
        with tempfile.TemporaryDirectory() as td, mock.patch.dict(os.environ, self._env(td), clear=True):
            runner = lambda args: proc(json.dumps(raw))
            fetched = um.fetch_changelog("0.8.1", runner=runner)
            self.assertEqual(fetched["body"], raw["body"])
            again = um.fetch_changelog("0.8.1", runner=mock.Mock(side_effect=AssertionError("cache")))
